"""
Customer Churn Pipeline with KFP OpenLineage Decorator

This version uses the @track_component_lineage decorator to automatically
emit OpenLineage events for each KFP component.

The decorator emits events directly to Marquez - no intermediate webhook needed.
"""
...
# Import decorator - this will be available in the component containers
def import_decorator():
    """Helper to import decorator inside components"""
    import sys
    sys.path.insert(0, '/opt/spark')  # For Spark image
    sys.path.insert(0, '/app')         # For FKM image
    from kfp_openlineage_decorator import track_component_lineage
    return track_component_lineage


# ============================================================================
# PLATFORM COMPONENTS (with decorator)
# ============================================================================
...
def platform_spark_etl(
    ...
    """
    PySpark ETL with automatic KFP-level lineage tracking via decorator.

    This emits THREE levels of lineage:
    1. KFP component-level (via @track_component_lineage decorator)
    2. Spark job-level (via native Spark OpenLineage listener)
    3. Dataset-level (automatically captured by Spark listener)
    """
    import os
    import subprocess
    import sys
    ...
    outputs = [
        {
            "namespace": "postgres://postgres:5432/warehouse",
            ...
            "facets": {
                "dataSource": {
                    ...
                    "_schemaURL": "https://openlineage.io/spec/facets/1-0-0/DataSourceDatasetFacet.json",
                    "name": "postgres",
                    "uri": f"postgresql://{pg_host}:5432/{pg_database}/{warehouse_table}"
                },
                "schema": {
                    "_producer": "https://github.com/practice-mlops/kfp-openlineage",
                    ...
                    "fields": [
                        ...
                        {"name": "tenure_months", "type": "double"},
                        {"name": "monthly_charges", "type": "double"},
                        {"name": "total_charges", "type": "double"},
                        {"name": "num_support_tickets", "type": "double"},
    ...
    # Wrap the actual work with the decorator
...
def platform_feast_apply(
    ...
    """Run feast apply with automatic lineage tracking"""
    import os
    import subprocess
    import sys
    ...
    outputs = [
        {
            "namespace": "feast://customer_churn",
            ...
            "facets": {
                "dataSource": {
                    ...
                    "_schemaURL": "https://openlineage.io/spec/facets/1-0-0/DataSourceDatasetFacet.json",
                    "name": "feast",
                    "uri": "feast://customer_churn/customer_features_view"
                }
            }
        }
    ]
    ...
    @track_component_lineage(
        namespace="churn-demo/customer_churn",
        inputs=inputs,
        outputs=outputs
    )
    def run_feast_apply():
...
def platform_feast_materialize(
    ...
    """Materialize features with automatic lineage tracking"""
    import os
    import sys
    ...
    outputs = [
        {
            ...
            "facets": {
                "dataSource": {
                    ...
                    "_schemaURL": "https://openlineage.io/spec/facets/1-0-0/DataSourceDatasetFacet.json",
                    "name": "redis",
                    "uri": f"redis://{redis_host}:6379/customer_features"
                }
            }
        }
    ]
...
""")
    ...
    return run_feast_materialize()


# ============================================================================
# DATA SCIENCE COMPONENTS (keeping existing without decorator for now)
# ============================================================================
...
""")
    ...
    for df_ in (features_df, entity_df):
        df_["event_timestamp"] = pd.to_datetime(df_["event_timestamp"], utc=True)
...
def ds_data_validation(
    ...
    """Validate data quality (keeps existing manual lineage emission)"""
    import json
    import os
    ...
    batch_def = data_asset.add_batch_definition_whole_dataframe("validation_batch")
    ...
    numeric_cols = ["tenure_months", "monthly_charges", "total_charges", "num_support_tickets"]
    for col in numeric_cols:
        suite.add_expectation(
            gx.expectations.ExpectColumnValuesToBeBetween(column=col, min_value=0, mostly=0.99)
        )

    suite.add_expectation(
        gx.expectations.ExpectColumnDistinctValuesToBeInSet(column="churn", value_set=[0, 1])
    )
    ...
    if not success:
        failed = [r.expectation_config.to_dict() for r in validation_result.results if not r.success]
        print(f"Failed expectations: {failed[:3]}")
        raise RuntimeError("Data validation failed")

    # Manual OpenLineage event emission
    run_id = str(uuid4())
    event = {
        "eventType": "COMPLETE",
        "eventTime": datetime.now(timezone.utc).isoformat(),
        "run": {"runId": run_id},
        "job": {"namespace": openlineage_namespace, "name": "ds_data_validation"},
        "inputs": [{
            "namespace": openlineage_namespace,
            ...
            "facets": {
                "dataQuality": {
                    ...
                    "rowCount": len(df),
                    "columnMetrics": {col: {"nullCount": int(df[col].isnull().sum())} for col in critical_cols}
                }
            }
        }],
        "outputs": [{"namespace": openlineage_namespace, "name": "customer_features_validated", "facets": {}}],
        "producer": "https://github.com/practice-mlops/great-expectations"
    }
...
def ds_model_training(
    ...
    feature_cols = [
        ...
        "num_support_tickets", "contract_type", "internet_service", "payment_method",
    ]
    ...
    with mlflow.start_run("namespace", "job_name") as run:
        train_dataset = mlflow.data.from_pandas(df, source=dataset.path "s3://pipline-artifeact/.....", name="customer_features_view")
...
def ds_model_registration(
    ...
    """Register model in MLflow if metrics meet threshold"""
    import json
    import os
    import mlflow
...
@dsl.pipeline(
    name="customer-churn-ml-pipeline-decorator",
    description=(
        "End-to-end churn prediction pipeline with automatic KFP OpenLineage integration. "
        "Uses @track_component_lineage decorator for automatic event emission."
    ),
)
def customer_churn_pipeline(
    ...
    # Platform tasks with automatic lineage
    ...
    # Set environment variables for parent context tracking
    spark_etl_task.set_env_variable("KFP_PIPELINE_NAME", "customer_churn_pipeline")
    spark_etl_task.set_env_variable("OPENLINEAGE_URL", "http://marquez.lineage.svc")
    ...
    feast_apply_task.set_env_variable("KFP_PIPELINE_NAME", "customer_churn_pipeline")
    feast_apply_task.set_env_variable("OPENLINEAGE_URL", "http://marquez.lineage.svc")
    ...
    feast_materialize_task.set_env_variable("KFP_PIPELINE_NAME", "customer_churn_pipeline")
    feast_materialize_task.set_env_variable("OPENLINEAGE_URL", "http://marquez.lineage.svc")
...
if __name__ == "__main__":
    compiler.Compiler().compile(
        ...
        package_path="customer_churn_pipeline_with_decorator.yaml",
    )
    print("Pipeline compiled successfully: customer_churn_pipeline_with_decorator.yaml")
    print("\nThis pipeline emits automatic KFP-level lineage using @track_component_lineage decorator")
    print("\nLineage layers:")
    print("  1. KFP component-level (via decorator) - tracks component execution")
    print("  2. Tool-specific lineage:")
    print("     - Spark: native OpenLineage listener")
    print("     - Feast: built-in OpenLineage support")
    print("     - MLflow: OpenLineage plugin")
    print("     - Great Expectations: manual emission")
