...
# Image stream namespace (match deploy namespace). Override via OPENSHIFT_APP_NAMESPACE.
_OCP_IMG_NS = os.environ.get("OPENSHIFT_APP_NAMESPACE", "lineage")
...
FKM_IMAGE = (
    f"image-registry.openshift-image-registry.svc:5000/{_OCP_IMG_NS}/fkm-app:latest"
)
SPARK_IMAGE = (
    f"image-registry.openshift-image-registry.svc:5000/{_OCP_IMG_NS}/spark-etl:latest"
)
...
def platform_feast_apply(
    ...
    fs_yaml = write_feast_feature_store_yaml(
        feast_repo_path,
...
def platform_feast_materialize(
    ...
    ol_namespace = os.environ.get("OPENLINEAGE_NAMESPACE", "default")
    feast_project = ol_namespace.replace("-", "_")
    write_feast_feature_store_yaml(
        feast_repo_path,
        feast_project=feast_project,
        pg_host=pg_host,
        redis_host=redis_host,
...
def ds_data_extraction(
    ...
    from src.pipeline import components

    _ = materialize_done  # KFP dependency edge only

    ol_namespace = os.environ.get("OPENLINEAGE_NAMESPACE", "default")
    print(f"DS: data extraction  pg={pg_host} redis={redis_host} ol_ns={ol_namespace}")
    ...
    with kfp_lineage(
        ...
        result = components.data_extraction_for_kfp(
            pg_url, feast_repo_path, table_name, pg_host, redis_host,
        )
...
def ds_data_validation(
    ...
    from src.pipeline.gx_churn_validation import validate_churn_dataset_with_gx

    df = pd.read_parquet(dataset.path)
    df = validate_churn_dataset_with_gx(df, openlineage_url, openlineage_namespace)
...
def ds_feature_engineering(
    ...
    with kfp_lineage(
        ...
        df = pd.read_parquet(dataset.path)
        df = components.feature_engineering(df)
...
def ds_model_training(
    ...
    os.environ["DATASET_SOURCE_URI"] = dataset.uri
    ...
    with kfp_lineage(
        ...
        df = pd.read_parquet(dataset.path)
        out = components.model_training(df, tracking_uri, experiment_name, params=None)
        metrics = {k: float(v) for k, v in out["metrics"].items()}
        print(f"DS: model training complete - metrics: {metrics}")
        return json.dumps({
            "run_id": out["run_id"],
            "model_uri": out["model_uri"],
            "metrics": metrics,
        })
...
# =======================================================================
@dsl.component(base_image=FKM_IMAGE)
def ds_evaluation(train_result_json: str, tracking_uri: str) -> str:
    ...
    with kfp_lineage(
        ...
        m = components.evaluation(result, tracking_uri=tracking_uri)
        return json.dumps({k: float(v) for k, v in m.items()})
...
def ds_model_registration(
    ...
    with kfp_lineage(
        ...
        d = components.model_registration(
            train_result, metrics, model_name, tracking_uri, roc_auc_threshold,
        )
        return json.dumps(d)
...
def customer_churn_pipeline(
    ...
    for task in [etl_task, apply_task, materialize_task, extract_task,
                 validate_task, engineer_task, train_task, eval_task, reg_task]:
